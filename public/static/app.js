/* ═══════════════════════════════════════════════════════════
   FLAPY app.js v6.0 — STABLE RELEASE
   - Исправлена ошибка SyntaxError (зависание заставки)
   - Скрыты кнопки "+" и "..." для гостей
   - Исправлен вызов телефона на ПК
   - Восстановлено меню профиля
═══════════════════════════════════════════════════════════ */
'use strict'; 
...
window.addEventListener('load', function () {
  ...
  updateNavVisibility(); // Управление кнопками меню
...
function updateNavVisibility() {
  ...
  // Логика: Если пользователь авторизован - показываем + и ...
  // Если гость - скрываем, оставляем только Объекты и Ленту
  if (curUser) {
...
function getFallbackListings() {
  return [
    ...
    { id:2, type:'apartment', rooms:3, area:82, district:'Есильский', city:'Астана', price:62000000, exchange:false, hasVideo:true, videoId:'tgbNymZ7vqY', realtor:'Данияр М.', realtorId:'r2', realtorFull:'Данияр Мусин', rating:4.7, deals:32, agency:'Etagi', tags:['Горящее'], badge:'Горящее', desc:'Отличная 3-комнатная.', photos:['🛋️','🚿'] },
  ];
}
...
function buildListCard(l) {
  ...
  // FIX: Убран рейтинг из карточки листа, как вы просили
  return (
...
function callRealtor(phone) {
  toast('📞 ' + phone);
  // Проверка: запускаем звонок только если это мобильное устройство, или если браузер разрешает
  // Для тестов просто покажем уведомление, чтобы не ломать работу
