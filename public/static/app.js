...
async function adminDeleteAll(){
  ...
  else{ alert('Удалено'); location.reload(); }
}

/* ════════════════════════════════════════════════════
   🚀 INIT — ИСПРАВЛЕНА ДЕСТРУКТУРИЗАЦИЯ
═══════════════════════════════════════════════════ */
...
async function submitListing(){
  ...
  const roomsEl = document.getElementById('a-rooms');
  const areaEl = document.getElementById('a-area');
  const cityEl = document.getElementById('a-city');
  const districtEl = document.getElementById('a-district');
  const tiktokEl = document.getElementById('a-tiktok');
  const exchangeCheck = document.getElementById('a-exchange');
  
  if(!priceEl || !descEl){
    showWarmToast('Заполните поля');
    return;
  }
  
  const price = parseInt(priceEl.value.replace(/\s/g,'')) || 0;
  const desc = descEl.value || '';
  const type = typeEl ? typeEl.value : 'apartment';
  const rooms = roomsEl ? parseInt(roomsEl.value) : 3;
  const area = areaEl ? parseInt(areaEl.value) : 85;
  const city = cityEl ? cityEl.value : 'Астана';
  const district = districtEl ? districtEl.value : 'Есиль';
  const tiktok = tiktokEl ? tiktokEl.value.trim() : '';
  const considerExchange = exchangeCheck ? exchangeCheck.checked : false;
  
  if(!desc || desc.trim() === ''){
    showWarmToast('Расскажите о доме');
    return;
  }
  if(price <= 0){
    showWarmToast('Укажите цену');
    return;
  }
  ...
  for(let i=0; i<uploadedMedia.photos.length; i++){
    watermarkedPhotos.push(await applyWatermark(uploadedMedia.photos[i]));
  }
...
function toggleFavorite(id){
  ...
  if(idx > -1){
    ...
    showWarmToast('🤍 Убрано');
  } else {
    favorites.push(id);
    showWarmToast('❤️ Сохранено');
  }
...
function renderListings(){
  ...
  el.innerHTML = filtered.map(function(l){
    ...
    const exchangeBadge = l.consider_exchange ? '<div style="position:absolute;top:10px;left:10px;padding:4px 10px;background:rgba(39,174,96,0.9);color:#fff;border-radius:6px;font-size:10px;font-weight:700">🔄 Обмен</div>' : '';
    const tiktokIcon = l.tiktok_url ? '<span style="margin-left:6px;color:#fe2c55">🎵</span>' : '';
    const photo = l.photo_urls && l.photo_urls[0] ? l.photo_urls[0] : null;
...
async function updateAuthUI(){
  ...
  if(curUser){
    ...
    const name = (curUser.user_metadata && curUser.user_metadata.full_name) ? curUser.user_metadata.full_name : email.split('@')[0];
    
    if(email.includes('admin') || curUser.role === 'superadmin'){
      btn.innerHTML = '👑';
      btn.style.background = 'linear-gradient(135deg,#667eea,#764ba2)';
      btn.onclick = showAdminPanelFromBtn;
    } else {
      btn.innerHTML = '👤 '+name.split(' ')[0];
      btn.style.background = 'var(--navy)';
      btn.onclick = function(){ go('s-prof'); };
    }
  } else {
    btn.innerHTML = WARM_WORDS.login;
    btn.style.background = 'var(--navy)';
    btn.onclick = function(){ openM('m-auth'); };
  }
}
...
async function doLogin(){
  ...
  if(!email || !pass){
    showWarmToast('Введите email и пароль');
    return;
  }
  ...
  if(error){
    if(error.message.includes('Email')){
      showWarmToast('❌ Подтвердите email (проверьте почту)');
    } else {
      showWarmToast('❌ '+error.message);
    }
    return;
...
async function doRegister(){
  ...
  if(!name || !email || !pass){
    showWarmToast('Заполните все поля');
    return;
  }
  ...
  if(error){
    if(error.message.includes('429')){
      showWarmToast('⏳ Подождите или используйте другой email');
    } else {
      showWarmToast('❌ '+error.message);
    }
    return;
...
async function doLogout(){
  ...
  localStorage.removeItem('fp_admin_session');
  await updateAuthUI();
  showWarmToast(WARM_WORDS.logout+' 💙');
  location.reload();
}

function renderAiraChat(){
  const el = document.getElementById('aira-msgs');
  if(!el) return;
  
  if(airaMessages.length === 0){
    el.innerHTML = '<div style="padding:60px 20px;text-align:center;color:var(--t3)"><div style="font-size:56px;margin-bottom:16px">💬</div><div style="font-size:16px;font-weight:600;margin-bottom:8px">Чат друзей</div><div style="font-size:13px">Здесь общаются риэлторы</div></div>';
    return;
  }
  
  el.innerHTML = airaMessages.map(function(m){
    const bg = m.mine ? 'var(--navy)' : '#fff';
    const color = m.mine ? '#fff' : 'var(--t1)';
    const align = m.mine ? 'me' : 'bot';
    
    return '<div style="display:flex;gap:8px;max-width:85%;margin-bottom:12px;'+(m.mine?'align-self:flex-end;flex-direction:row-reverse':'align-self:flex-start')+'"><div style="padding:11px 15px;border-radius:16px;font-size:14px;'+(m.mine?'background:'+bg+';color:'+color+';border-radius:16px 4px 16px 16px':'background:'+bg+';color:'+color+';border-radius:4px 16px 16px 16px')+'">'+m.text+'</div><div style="font-size:10px;color:var(--t3);margin-top:4px">'+m.time+'</div></div>';
  }).join('');
...
function sendAira(){
  const inp = document.getElementById('aira-inp');
  ...
  airaMessages.push({
    ...
    author: curUser ? curUser.email.split('@')[0] : 'Гость',
    text: txt,
    time: tm,
    mine: true
  });
...
function showWarmToast(msg){
  ...
  if(!el){
    ...
    el.id = 'toast';
    el.style.cssText = 'position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;padding:14px 28px;border-radius:14px;z-index:10000;opacity:0;transition:all 0.3s;font-weight:600;box-shadow:0 8px 30px rgba(102,126,234,0.4);font-size:14px;pointer-events:none';
...
function authTab(tab){
  const inTab = document.getElementById('at-in');
  const upTab = document.getElementById('at-up');
  const inForm = document.getElementById('af-in');
  const upForm = document.getElementById('af-up');
...
function setListTab(tab){
  listTab = tab;
  const t1 = document.getElementById('tab-obj');
  const t2 = document.getElementById('tab-exch');
...
function uploadMedia(){
  ...
  input.onchange = function(e){
    ...
    showWarmToast('✅ Добавлено: '+uploadedMedia.photos.length+'/5');
  };
