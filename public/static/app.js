...
window.addEventListener('load', async () => {
  ...
  if (session) {
    ...
    document.getElementById('auth-btn').textContent = '👤 ' + (curUser.email.split('@')[0]);
  }
...
function renderFeed() {
  ...
  main.innerHTML = filtered.map(l => {
    ...
    <div class="lcard" onclick="viewListing('${l.id}')">
      ...
      <div style="font-size:14px;color:var(--t2);margin-bottom:12px">${l.city} · ${l.rooms}-комн. · ${l.area} м²</div>
      <div style="font-size:13px;line-height:1.5;color:var(--t1);margin-bottom:16px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">${l.desc}</div>
...
async function submitListing() {
  ...
  // Модерация
  const spamWords = ['бля','сука','хуй','пизд','казино','спам'];
  if (spamWords.some(w => desc.toLowerCase().includes(w))) {
    alert('🚫 Обнаружена нецензурная лексика или спам.'); return;
  }
  if (!desc || desc.length < 10) { alert('📝 Описание слишком короткое.'); return; }
  
  const price = parseInt(priceStr);
  if (!price || price < 500000) { alert('💰 Минимальная цена 500 000 ₸'); return; }
...
async function viewListing(id) {
  ...
  // Increment views locally & in DB
  listing.views = (listing.views || 0) + 1;
  db.from('listings').update({ views: listing.views }).eq('id', id);
...
// Auth stubs (Supabase handles real auth, this is UI toggle)
