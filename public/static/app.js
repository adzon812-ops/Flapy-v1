/* FLAPY app.js v13.0 — PRODUCTION READY */
'use strict';
...
window.addEventListener('load',function(){
  ...
  renderListings();
  console.log('✅ Flapy app.js v13.0 loaded');
});
...
function saveListings(){try{localStorage.setItem('fp_listings',JSON.stringify(listings));}catch(e){}}
  ...
  input.onchange=function(e){
    ...
    Array.from(files).forEach(function(file){
      ...
      reader.onload=function(evt){
        if(type==='photo'){if(!uploadedMedia.photos)uploadedMedia.photos=[];uploadedMedia.photos.push(evt.target.result);}
        else{if(!uploadedMedia.videos)uploadedMedia.videos=[];uploadedMedia.videos.push(evt.target.result);}
...
function doLogout(){curUser=null;renderAuthSlot();renderProf();updateAiraBadge();updateNavVisibility();toast('👋 До встречи!');}
