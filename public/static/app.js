...
var T = {
  ru: {
    ...
    chip_desc: '✍️ Описание',
    chip_mortgage: '🏦 Ипотека',
    chip_promo: '📢 Продвижение',
    chip_tax: '💡 Налоги',
    chip_show: '📅 Показ',
    chip_val: '💰 Оценка',
    chip_exch: '🔄 Обмен',
...
function getFallbackListings() {
  return [
    { id:1, type:'apartment', rooms:3, area:85, district:'Бостандыкский', city:'Алматы', price:78500000, exchange:false, hasVideo:true, videoId:'ScMzIvxBSi4', realtor:'Айгерим К.', realtorId:'r1', realtorFull:'Айгерим Касымова', rating:4.9, deals:47, agency:'Century 21', tags:['Новострой'], badge:'Новое', desc:'Просторная 3-комнатная с панорамным видом.', photos:['🛋️','🛁','🪟','🏗️'] },
    { id:2, type:'apartment', rooms:3, area:82, district:'Есильский', city:'Астана', price:62000000, exchange:false, hasVideo:true, videoId:'tgbNymZ7vqY', realtor:'Данияр М.', realtorId:'r2', realtorFull:'Данияр Мусин', rating:4.7, deals:32, agency:'Etagi', tags:['Горящее'], badge:'Горящее', desc:'Отличная 3-комнатная в новом ЖК.', photos:['🛋️','🚿','🌇'] },
    { id:3, type:'house', rooms:5, area:220, district:'Алматинский', city:'Астана', price:150000000, exchange:true, hasVideo:true, videoId:'UxxajLWwzqY', realtor:'Сауле Т.', realtorId:'r3', realtorFull:'Сауле Тлеубекова', rating:5.0, deals:68, agency:'Royal Group', tags:['Обмен'], badge:'Обмен', desc:'Дом с участком 10 соток.', photos:['🏡','🌳','🏊','🔥'] },
    { id:4, type:'commercial', rooms:0, area:120, district:'Байконыр', city:'Астана', price:65000000, exchange:false, hasVideo:false, videoId:'', realtor:'Нурлан А.', realtorId:'r4', realtorFull:'Нурлан Ахметов', rating:4.6, deals:23, agency:'Самозанятый', tags:['Инвест'], badge:'Топ', desc:'Помещение первой линии.', photos:['🏪','📐','🔌'] },
    { id:5, type:'apartment', rooms:2, area:65, district:'Сарыарка', city:'Астана', price:38000000, exchange:true, hasVideo:false, videoId:'', realtor:'Айгерим К.', realtorId:'r1', realtorFull:'Айгерим Касымова', rating:4.9, deals:47, agency:'Century 21', tags:['Обмен'], badge:'Обмен', desc:'Уютная 2-комнатная.', photos:['🛋️','🚿'] },
    { id:6, type:'apartment', rooms:1, area:42, district:'Есиль', city:'Астана', price:29000000, exchange:false, hasVideo:true, videoId:'jNQXAC9IVRw', realtor:'Данияр М.', realtorId:'r2', realtorFull:'Данияр Мусин', rating:4.7, deals:32, agency:'Etagi', tags:['Студия'], badge:'Новое', desc:'Стильная студия.', photos:['🛋️','🌃'] }
  ];
}

function getFallbackRealtors() {
  return [
    { id:'r1', name:'Айгерим Касымова', agency:'Century 21', rating:4.9, deals:47, reviews:23, phone:'+7 701 234 56 78', photo:'А', color:'#1E2D5A', specialization:'Квартиры, новострой', experience:5, badge:'ТОП', verified:true, reviewsList:[ {author:'Алия С.', stars:5, text:'Профессионал!', date:'15 янв'}, {author:'Марат Б.', stars:5, text:'Рекомендую!', date:'3 янв'} ] },
    { id:'r2', name:'Данияр Мусин', agency:'Etagi', rating:4.7, deals:32, reviews:18, phone:'+7 702 345 67 89', photo:'Д', color:'#F47B20', specialization:'Дома, коттеджи', experience:7, badge:'', verified:true, reviewsList:[ {author:'Самал Т.', stars:5, text:'Отличный риэлтор!', date:'10 янв'} ] },
    { id:'r3', name:'Сауле Тлеубекова', agency:'Royal Group', rating:5.0, deals:68, reviews:41, phone:'+7 707 456 78 90', photo:'С', color:'#27AE60', specialization:'Коммерция', experience:9, badge:'ТОП', verified:true, reviewsList:[ {author:'ТОО Казсервис', stars:5, text:'Лучший риэлтор!', date:'20 янв'} ] },
    { id:'r4', name:'Нурлан Ахметов', agency:'Самозанятый', rating:4.6, deals:23, reviews:12, phone:'+7 705 567 89 01', photo:'Н', color:'#9B59B6', specialization:'Обмен, любые', experience:3, badge:'', verified:true, reviewsList:[ {author:'Гульнара М.', stars:4, text:'Помог с обменом', date:'8 янв'} ] },
    { id:'r5', name:'Асель Бекова', agency:'Etagi', rating:4.8, deals:38, reviews:19, phone:'+7 708 678 90 12', photo:'А', color:'#E67E22', specialization:'Новострой', experience:4, badge:'', verified:true, reviewsList:[ {author:'Болат С.', stars:5, text:'Помогла купить!', date:'2 янв'} ] }
  ];
}
...
function openDetail(id) {
  ...
  var realtorHtml = '<div class="det-realtor" onclick="openRealtorProfile(\''+l.realtorId+'\')">' +
    ...
    '<div style="flex:1"><div style="font-size:13px;font-weight:700">'+esc(l.realtorFull||l.realtor||'')+'</div><div style="font-size:11px;color:var(--t3)">'+esc(l.agency||'')+' · '+l.deals+' сделок</div></div>' +
    '<div style="font-size:11px;color:var(--navy);font-weight:600">Профиль →</div>' +
  '</div>';
  
  document.getElementById('m-det-body').innerHTML = '<div class="sh-handle"></div>' + visualHtml + photosHtml +
    ...
    exchHtml + '<div class="det-desc">'+(l.desc||'').replace(/\n/g,'<br>')+'</div>' + realtorHtml +
    '<div class="det-cta">' +
    ...
    '</div>' +
    '<div style="padding:0 17px 4px"><button class="btn-outline" onclick="openHireModal('+l.id+')">🤝 '+t('hire_btn')+' риэлтора для продажи</button></div>';
  openM('m-det');
}
...
function goChat(id) {
  ...
  if (l) {
    setTimeout(function(){
      ...
      if (inp) { inp.value = 'Интересует объект: '+(l.rooms?l.rooms+'к, ':'')+esc(l.district)+', '+fmtPrice(l.price)+' ₸'; inp.focus(); autoResize(inp); }
    }, 200);
  }
}
...
function updateNavVisibility() {
  ...
  if (moreBtn) {
    if (curUser) { moreBtn.style.display = 'flex'; } else { moreBtn.style.display = 'none'; }
  }
}
...
function addAiraThread(txt, type) {
  ...
  div.innerHTML = '<div class="th-head" onclick="toggleThread(this)">' +
    ...
    '<div style="display:flex;gap:6px"><button onclick="replyAira(this)" style="padding:5px 10px;border-radius:7px;background:var(--navy);color:#fff;font-size:11px;font-weight:600;cursor:pointer">💬 Ответить</button></div></div>';
...
function renderCal() {
  ...
  function evHtml(e) {
    ...
    return '<div class="ev-card" onclick="editEvent('+e.id+')">' +
      ...
      '<div class="ev-inf"><div class="ev-ttl">'+ic+' '+esc(e.title)+'</div>' +
      (e.client?'<div class="ev-cli">👤 '+esc(e.client)+'</div>':'') +
      (e.note?'<div class="ev-note">'+esc(e.note)+'</div>':'') +
      '</div><button onclick="event.stopPropagation();deleteEvent('+e.id+')" style="flex-shrink:0;background:none;font-size:14px;color:var(--t3);cursor:pointer;padding:4px">✕</button></div>';
  }
  var html = '<div class="cal-title">📅 '+t('nav_obj')+' / Расписание</div>' +
    '<div class="cal-date">'+dStr+'</div>' +
    '<div class="ai-tip"><span style="font-size:18px">🤖</span><span><b>Flai:</b> '+(todayEv.length?'У вас '+todayEv.length+' событий сегодня. Не забудьте подготовиться!':'Сегодня событий нет. Хотите запланировать показ?')+'</span></div>' +
...
function renderRatingBlock() {
  ...
  return list.slice(0,5).map(function(r,i) {
    ...
    return '<div class="rank-card" onclick="'+(realtors.length?'openRealtorProfile(\''+r.id+'\')':'renderRealtors()')+'">' +
      ...
      '<div style="flex:1"><div style="font-size:13px;font-weight:700">'+esc(r.name||r.realtorFull||'')+'</div>' +
      '<div class="rank-bar" style="width:'+w+'%"></div>' +
      '<div style="font-size:11px;color:var(--t3);margin-top:3px">'+r.deals+' сделок · ⭐ '+r.rating+'</div></div></div>';
  }).join('');
}
...
function buildRealtorCard(r,i) {
  ...
  return '<div class="rcard" onclick="openRealtorProfile(\''+r.id+'\')">' +
    ...
    '<div class="rc-stats"><div class="rc-stat"><b>'+r.deals+'</b> сделок</div><div class="rc-stat"><b>'+r.experience+' лет</b> опыта</div></div>' +
    ...
    '<div class="rc-actions"><button class="rc-btn rc-call" onclick="event.stopPropagation();callRealtor(\''+esc(r.phone)+'\')"><i class="fas fa-phone"></i> Позвонить</button>' +
    ...
    '<button class="rc-btn rc-hire" onclick="event.stopPropagation();hireRealtor(\''+r.id+'\',\''+esc(r.name)+'\')"><i class="fas fa-handshake"></i> Нанять</button></div></div></div>';
}
...
function openRealtorProfile(id) {
  ...
  var reviewsHtml = (r.reviewsList||[]).map(function(rv){
    return '<div class="review-item"><div class="rev-head"><div class="rev-ava">'+rv.author.charAt(0)+'</div><div class="rev-name">'+esc(rv.author)+'</div><div class="rev-stars">'+rv.stars+' ★</div></div>' +
      '<div class="rev-text">'+esc(rv.text)+'</div><div style="font-size:10px;color:var(--t3);margin-top:4px">'+esc(rv.date)+'</div></div>';
  }).join('');
  ...
  body.innerHTML = '<div class="sh-handle"></div>' +
    '<div style="background:linear-gradient(135deg,'+r.color+','+r.color+'cc);padding:20px 17px 16px;text-align:center">' +
      ...
      '<div style="font-size:18px;font-weight:800;color:#fff">'+r.rating+' / 5.0</div></div>' +
    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;padding:13px 17px">' +
      ...
      '<div style="background:var(--bg3);border-radius:10px;padding:11px;text-align:center"><div style="font-size:17px;font-weight:800;color:var(--navy)">'+r.experience+' л</div><div style="font-size:10px;color:var(--t3)">Опыт</div></div></div>' +
    '<div style="padding:0 17px"><div style="font-size:12px;font-weight:600;color:var(--t3);margin-bottom:5px">Специализация</div><div style="font-size:13px;color:var(--t1)">'+esc(r.specialization)+'</div></div>' +
    '<div style="display:flex;gap:8px;padding:13px 17px"><button class="det-btn det-call" onclick="callRealtor(\''+esc(r.phone)+'\')"><i class="fas fa-phone"></i> Позвонить</button>' +
    '<button class="det-btn det-chat" onclick="closeM(\'m-realtor\');chatWithRealtor(\''+r.id+'\',\''+esc(r.name)+'\')"><i class="fas fa-comment"></i> Написать</button></div>' +
...
function renderHireModal(listingId) {
  ...
  if (!body) return;
  body.innerHTML = '<div style="font-size:12px;color:var(--t2);margin-bottom:14px">'+t('choose_realtor')+'</div>' +
    realtors.map(function(r){
      ...
      return '<div class="rcard" style="margin-bottom:10px;'+(isSelected?'border:2px solid var(--green)':'')+'" onclick="hireRealtor(\''+r.id+'\',\''+esc(r.name)+'\')">' +
        (r.badge?'<div class="rc-badge">'+r.badge+'</div>':'') +
        '<div class="rc-ava" style="background:'+r.color+'">'+r.photo+'</div>' +
        '<div style="flex:1"><div class="rc-name">'+esc(r.name)+(isSelected?' <span style="color:#27AE60;font-size:11px">✓ Текущий</span>':'')+'</div>' +
        '<div class="rc-agency">'+esc(r.agency)+'</div>' +
        '<div class="rc-stars">★ '+r.rating+'<span>'+r.reviews+' '+t('reviews_label')+'</span></div>' +
        '<div class="rc-stats"><div class="rc-stat"><b>'+r.deals+'</b> '+t('deals_label')+'</div><div class="rc-stat"><b>'+r.experience+' лет</b> опыта</div></div>' +
        '<div class="rc-spec">'+esc(r.specialization)+'</div></div></div>';
    }).join('') +
...
function openExchangeModal(listingId) {
  ...
  if (!body) return;
  body.innerHTML = '<div style="padding:0 0 6px"><div style="font-size:13px;color:var(--t2);margin-bottom:12px">Ваш объект: <b>'+esc(l.rooms?l.rooms+'к ':'')+'</b> · '+esc(l.district)+' · '+fmtPrice(l.price)+' ₸</div></div>' +
    ...
    matches.map(function(m){
      ...
      return '<div class="lcard" style="margin-bottom:10px;cursor:pointer" onclick="proposeExchangeDeal('+l.id+','+m.id+')">' +
        '<div class="lcard-body"><div class="lcard-loc"><i class="fas fa-map-marker-alt"></i>'+esc(m.city)+', '+esc(m.district)+'</div>' +
        ...
        '<div class="lcard-footer"><div class="lf-ava" style="background:#27AE60">'+esc((m.realtorFull||m.realtor||'R').charAt(0))+'</div>' +
        '<div class="lf-name">'+esc(m.realtorFull||'')+' · '+esc(m.agency||'')+'</div><div class="lf-rating">★ '+m.rating+'</div></div>' +
        '<button class="btn-primary" style="margin-top:8px" onclick="event.stopPropagation();proposeExchangeDeal('+l.id+','+m.id+')">🔄 Предложить обмен</button></div></div>';
    }).join('') +
...
function submitListing() {
  ...
  var videoId = '';
  if (videoU) { var ym = videoU.match(/(?:v=|youtu\.be\/)([A-Za-z0-9_-]{11})/); if (ym) videoId = ym[1]; }
...
function addMsg(cid, txt, mine, ini) {
  ...
  if (mine) { div.innerHTML = '<div class="bwrap"><div class="bubble">'+fmt+'</div><div class="m-ts">'+now+' ✓</div></div>'; }
  else { div.innerHTML = '<div class="m-ava">'+(ini||'AI')+'</div><div class="bwrap"><div class="bubble">'+fmt+'</div><div class="m-ts">'+now+'</div></div>'; }
  c.appendChild(div);
...
function sendFlaiMsg(txt) {
  ...
  fetch('/api/chat/flai', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({message:txt, lang:curLang, role:curUser?'realtor':'buyer'}) })
    .then(function(r){ return r.json(); }).then(function(d) {
      ...
      typing.remove(); addMsg('flai-msgs', getFlaiLocalReply(txt), false, 'F');
    });
}
...
function getFlaiLocalReply(msg) {
  ...
  if (m.includes('цена') || m.includes('баға') || m.includes('сколько') || m.includes('стоимость')) return kz ? '💰 Баға ауданға, ауданға және жай-күйіне байланысты. Есіл — 1к от 28 млн. Нақты баға беруді қалайсыз ба?' : '💰 Цена зависит от района, площади и состояния. В Есиле 1к от 28 млн ₸. Хотите оценку конкретного объекта?';
