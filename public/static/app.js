...
function getFallbackRealtors() {
  return [
    {id:'r1', name:'Айгерим Касымова', agency:'Century 21', rating:4.9, deals:47, reviews:23, phone:'+7 701 234 56 78', photo:'А', color:'#1E2D5A', specialization:'Квартиры, новострой', experience:5, badge:'ТОП', verified:true, reviewsList:[
    ...
    {id:'r2', name:'Данияр Мусин', agency:'Etagi', rating:4.7, deals:32, reviews:18, phone:'+7 702 345 67 89', photo:'Д', color:'#F47B20', specialization:'Дома, коттеджи', experience:7, badge:'', verified:true, reviewsList:[
    ...
    {id:'r3', name:'Сауле Тлеубекова', agency:'Royal Group', rating:5.0, deals:68, reviews:41, phone:'+7 707 456 78 90', photo:'С', color:'#27AE60', specialization:'Коммерция', experience:9, badge:'ТОП', verified:true, reviewsList:[
    ...
    {id:'r4', name:'Нурлан Ахметов', agency:'Самозанятый', rating:4.6, deals:23, reviews:12, phone:'+7 705 567 89 01', photo:'Н', color:'#9B59B6', specialization:'Обмен, любые объекты', experience:3, badge:'', verified:true, reviewsList:[
      {author:'Гульнара М.', stars:4, text:'Помог с обменом, всё прошло гладко.', date:'8 янв'}
    ]},
    {id:'r5', name:'Асель Бекова', agency:'Etagi', rating:4.8, deals:38, reviews:19, phone:'+7 708 678 90 12', photo:'А', color:'#E67E22', specialization:'Новострой', experience:4, badge:'', verified:true, reviewsList:[
...
function openExchangeModal(listingId) {
  ...
  body.innerHTML = 
    ...
    matches.map(function(m) {
      ...
      var diffTxt = diff > 0 ? ' (+'+fmtPrice(diff)+' ₸ доплата)' : diff < 0 ? ' ('+fmtPrice(Math.abs(diff))+' ₸ экономия)' : ' (равноценный обмен)';
...
function openRealtorProfile(id) {
  ...
  if(!r) { 
    fetchRealtors(function(){ openRealtorProfile(id); }); 
    return; 
  }
...
function getFlaiLocalReply(msg) {
  ...
  if(m.includes('цена') || m.includes('баға') || m.includes('сколько') || m.includes('стоимость'))
    return kz ? '💰 Баға ауданға, ауданына және жай-күйіне байланысты. Есіл — 1к от 28 млн. Нақты баға беруді қалайсыз ба?' : '💰 Цена зависит от района, площади и состояния. В Есиле 1к от 28 млн ₸. Хотите оценку конкретного объекта?';
...
function saveEv() {
  ...
  ['ev-title','ev-client','ev-note'].forEach(function(id){ 
    var e = document.getElementById(id); 
    if(e) e.value = ''; 
  });
