...
function openDetail(id) {
  ...
  '</div>';
  document.getElementById('m-det-body').innerHTML = '<div class="sh-handle"></div>' + visualHtml + photosHtml + '<div class="det-price">'+pr+' ₸</div>' + exH + '<div class="det-grid">'+rmH+arH + '<div class="det-cell"><div class="det-val">'+esc(l.district||'')+'</div><div class="det-lbl">'+t('district')+'</div></div>' + '<div class="det-cell"><div class="det-val">⭐ '+l.rating+'</div><div class="det-lbl">Рейтинг</div></div>' + '</div>' + exchHtml + '<div class="det-desc">'+(l.desc||'').replace(/\n/g,'<br>')+'</div>' + realtorHtml + '<div class="det-cta">' + '<button class="det-btn det-call" onclick="callRealtor(\''+(l.phone||'+7 701 234 56 78')+'\')"><i class="fas fa-phone"></i> Позвонить</button>' + '<button class="det-btn det-chat" onclick="closeM(\'m-det\');goChat('+l.id+')"><i class="fas fa-comment"></i> Написать</button>' + '</div>' + '<div style="padding:0 17px 4px">' + '<button class="btn-outline" onclick="openHireModal(\''+l.id+'\')">🤝 '+t('hire_btn')+' риэлтора для продажи</button>' + '</div>';
  openM('m-det');
}
...
function openExchangeModal(listingId) {
  ...
  if(!body) return;
  body.innerHTML = '<div style="padding:0 0 6px"><div style="font-size:13px;color:var(--t2);margin-bottom:12px">Ваш объект: <b>'+esc(l.rooms?l.rooms+'к ':'')+'</b> · '+esc(l.district)+' · '+fmtPrice(l.price)+' ₸</div></div>' + '<div style="font-size:12px;font-weight:700;color:var(--t3);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Подходящие объекты:</div>' + matches.map(function(m) { var diff = m.price - l.price; var diffTxt = diff > 0 ? ' (+'+fmtPrice(diff)+' ₸ доплата)' : diff < 0 ? ' ('+fmtPrice(diff)+' ₸ экономия)' : ' (равноценный обмен)'; return '<div class="lcard" style="margin-bottom:10px;cursor:pointer" onclick="proposeExchangeDeal('+l.id+','+m.id+')">' + '<div class="lcard-body">' + '<div class="lcard-loc"><i class="fas fa-map-marker-alt"></i>'+esc(m.city)+', '+esc(m.district)+'</div>' + '<div class="lcard-price">'+fmtPrice(m.price)+' ₸</div>' + '<div class="lcard-sub">'+(m.rooms?m.rooms+'-комнатная, ':'')+m.area+' м²</div>' + '<div style="font-size:12px;color:#27AE60;font-weight:600;margin-top:4px">🔄 '+diffTxt+'</div>' + '<div class="lcard-footer">' + '<div class="lf-ava" style="background:#27AE60">'+esc((m.realtorFull||m.realtor||'R').charAt(0))+'</div>' + '<div class="lf-name">'+esc(m.realtorFull||'')+' · '+esc(m.agency||'')+'</div>' + '<div class="lf-rating">★ '+m.rating+'</div>' + '</div>' + '<button class="btn-primary" style="margin-top:8px" onclick="event.stopPropagation();proposeExchangeDeal('+l.id+','+m.id+')">🔄 Предложить обмен</button>' + '</div>' + '</div>'; }).join('') + '<div class="info-box" style="margin-top:4px"><span>💡</span><span>В 2026 году обмен освобождает от налога с продажи до <b>10–15%</b> от стоимости!</span></div>';
  openM('m-exchange');
}
...
function openHireModal(listingId) {
  closeM('m-det');
  curHireId = listingId;
  if(!realtors.length) { fetchRealtors(function(){ renderHireModal(listingId); }); }
  else { renderHireModal(listingId); }
}
function renderHireModal(listingId) {
  ...
  if(!body) return;
  body.innerHTML = '<div style="font-size:12px;color:var(--t2);margin-bottom:14px">'+t('choose_realtor')+'</div>' + realtors.map(function(r) { var isSelected = l && l.realtorId === r.id; return '<div class="rcard" style="margin-bottom:10px;'+(isSelected?'border:2px solid var(--green)':'')+'" onclick="hireRealtor(\''+r.id+'\',\''+esc(r.name)+'\')">' + (r.badge ? '<div class="rc-badge">'+r.badge+'</div>' : '') + '<div class="rc-ava" style="background:'+r.color+'">'+r.photo+'</div>' + '<div style="flex:1">' + '<div class="rc-name">'+esc(r.name)+(isSelected?' <span style="color:#27AE60;font-size:11px">✓ Текущий</span>':'')+'</div>' + '<div class="rc-agency">'+esc(r.agency)+'</div>' + '<div class="rc-stars">★ '+r.rating+'<span>'+r.reviews+' '+t('reviews_label')+'</span></div>' + '<div class="rc-stats">' + '<div class="rc-stat"><b>'+r.deals+'</b> '+t('deals_label')+'</div>' + '<div class="rc-stat"><b>'+r.experience+' лет</b> опыта</div>' + '</div>' + '<div class="rc-spec">'+esc(r.specialization)+'</div>' + '</div>' + '</div>'; }).join('') + '<div class="info-box warn" style="margin-top:6px"><span>🏆</span><span>Риэлтор получает комиссию только после успешной сделки. Вы можете сменить риэлтора в любой момент.</span></div>';
  openM('m-hire');
}
function hireRealtor(realtorId, realtorName) {
  ...
  if(l) {
    ...
    if(r) {
      l.realtorId = r.id; l.realtor = r.name.split(' ')[0]+' '+r.name.split(' ')[1]?.charAt(0)+'.'; l.realtorFull = r.name; l.agency = r.agency; l.rating = r.rating; l.deals = r.deals; l.realtorColor = r.color;
    }
  }
  closeM('m-hire');
  if(curUser) { addAiraThread('🤝 Назначен риэлтор: '+esc(realtorName)+' для моего объекта', 'listing'); }
  fetch('/api/listing/rate-realtor', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ realtorId: realtorId, listingId: curHireId }) }).catch(function(){});
...
function openRealtorProfile(id) {
  ...
  var reviewsHtml = (r.reviewsList||[]).map(function(rv) {
    return '<div class="review-item">' + '<div class="rev-head"><div class="rev-ava">'+rv.author.charAt(0)+'</div><div class="rev-name">'+esc(rv.author)+'</div><div class="rev-stars">'+rv.stars+' ★</div></div>' + '<div class="rev-text">'+esc(rv.text)+'</div>' + '<div style="font-size:10px;color:var(--t3);margin-top:4px">'+esc(rv.date)+'</div>' + '</div>';
  }).join('');
  var body = document.getElementById('m-realtor-body');
  if(!body) return;
  body.innerHTML = '<div class="sh-handle"></div>' + '<div style="background:linear-gradient(135deg,'+r.color+','+r.color+'cc);padding:20px 17px 16px;text-align:center">' + '<div style="width:64px;height:64px;border-radius:50%;background:rgba(255,255,255,.2);border:2px solid rgba(255,255,255,.4);display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:900;color:#fff;margin:0 auto 10px">'+r.photo+'</div>' + '<div style="font-size:18px;font-weight:800;color:#fff">'+esc(r.name)+'</div>' + '<div style="font-size:12px;color:rgba(255,255,255,.7);margin-top:2px">'+esc(r.agency)+(r.verified?' · ✓ Верифицирован':'')+'</div>' + '<div style="font-size:22px;color:#FFD700;margin-top:8px">'+stars+'</div>' + '<div style="font-size:18px;font-weight:800;color:#fff">'+r.rating+' / 5.0</div>' + '</div>' + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;padding:13px 17px">' + '<div style="background:var(--bg3);border-radius:10px;padding:11px;text-align:center"><div style="font-size:17px;font-weight:800;color:var(--navy)">'+r.deals+'</div><div style="font-size:10px;color:var(--t3)">Сделок</div></div>' + '<div style="background:var(--bg3);border-radius:10px;padding:11px;text-align:center"><div style="font-size:17px;font-weight:800;color:var(--navy)">'+r.reviews+'</div><div style="font-size:10px;color:var(--t3)">Отзывов</div></div>' + '<div style="background:var(--bg3);border-radius:10px;padding:11px;text-align:center"><div style="font-size:17px;font-weight:800;color:var(--navy)">'+r.experience+' л</div><div style="font-size:10px;color:var(--t3)">Опыт</div></div>' + '</div>' + '<div style="padding:0 17px">' + '<div style="font-size:12px;font-weight:600;color:var(--t3);margin-bottom:5px">Специализация</div>' + '<div style="font-size:13px;color:var(--t1)">'+esc(r.specialization)+'</div>' + '</div>' + '<div style="display:flex;gap:8px;padding:13px 17px">' + '<button class="det-btn det-call" onclick="callRealtor(\''+esc(r.phone)+'\')"><i class="fas fa-phone"></i> Позвонить</button>' + '<button class="det-btn det-chat" onclick="closeM(\'m-realtor\');chatWithRealtor(\''+r.id+'\',\''+esc(r.name)+'\')"><i class="fas fa-comment"></i> Написать</button>' + '</div>' + '<button class="btn-primary" style="margin:0 17px 8px;width:calc(100% - 34px)" onclick="closeM(\'m-realtor\');hireRealtor(\''+r.id+'\',\''+esc(r.name)+'\')">🤝 Нанять для продажи</button>' + '<button class="btn-outline" style="margin:0 17px 8px;width:calc(100% - 34px)" onclick="openRateModal(\''+r.id+'\',\''+esc(r.name)+'\')">⭐ Оставить отзыв</button>' + '<div style="padding:0 17px 17px">' + '<div style="font-size:12px;font-weight:700;color:var(--t3);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Отзывы клиентов</div>' + reviewsHtml + '</div>';
  openM('m-realtor');
}
function chatWithRealtor(realtorId, realtorName) {
  ...
  setTimeout(function(){
    var inp = document.getElementById('flai-inp');
    if(inp) { inp.value = 'Хочу связаться с риэлтором '+realtorName; inp.focus(); autoResize(inp); }
  }, 200);
...
function sendFlaiMsg(txt) {
  ...
  fetch('/api/chat/flai', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ message:txt, lang:curLang, role: curUser ? 'realtor' : 'buyer'}) })
  .then(function(r){ return r.json(); })
  .then(function(d) { typing.remove(); addMsg('flai-msgs', d.reply, false, 'F'); })
  .catch(function() { typing.remove(); addMsg('flai-msgs', getFlaiLocalReply(txt), false, 'F'); });
}
function getFlaiLocalReply(msg) {
  var m = msg.toLowerCase();
  var kz = curLang === 'kz';
  if(m.includes('обмен') || m.includes('айырбас')) return kz ? '🔄 Айырбас 2026 жылы өте тиімді! Салықсыз мерзім — 2 жыл. Бірнеше айырбас нұсқалары бар, қызығасыз ба?' : '🔄 Обмен в 2026 очень выгоден! Срок без налога — 2 года. Есть несколько подходящих вариантов для обмена, хотите посмотреть?';
  if(m.includes('ипотека') || m.includes('несие') || m.includes('кредит')) return kz ? '🏦 Отбасы Банк, Halyk Bank, Jusan Bank-пен жұмыс істейміз. Ставка жылдық 5%-дан. Нақты объект бойынша есептеп берейін?' : '🏦 Работаем с Отбасы Банк, Halyk Bank, Jusan Bank. Ставки от 5% годовых. Рассчитать ипотеку по конкретному объекту?';
  if(m.includes('цена') || m.includes('баға') || m.includes('сколько') || m.includes('стоимость')) return kz ? '💰 Баға ауданға, ауданына және жай-күйіне байланысты. Есіл — 1к от 28 млн. Нақты баға беруді қалайсыз ба?' : '💰 Цена зависит от района, площади и состояния. В Есиле 1к от 28 млн ₸. Хотите оценку конкретного объекта?';
  if(m.includes('налог') || m.includes('салық')) return kz ? '💡 2026 жылдан: салықсыз мерзім — 2 жыл. Мерзімінен бұрын сатқанда 10–15%. Айырбас — үнемдеудің тиімді жолы!' : '💡 С 2026 года: срок без налога — 2 года. При продаже раньше — 10–15%. Обмен — выгодная альтернатива!';
  if(m.includes('показ') || m.includes('көрсет') || m.includes('посмотреть')) return kz ? '📅 Ыңғайлы уақытта көрсетуді ұйымдастырамыз. Бүгін немесе ертең ыңғайлы ма?' : '📅 Организуем показ в удобное время. Когда удобно — сегодня или завтра?';
  if(m.includes('описание') || m.includes('сипаттама') || m.includes('составить')) return kz ? '✍️ Объект сипаттамасын дайындауға көмектесемін! Тип, ауданы мен бағасын айтыңыз.' : '✍️ Помогу составить привлекательное описание! Укажите тип, площадь, район и цену.';
  if(m.includes('продвижение') || m.includes('жылжыту') || m.includes('реклама')) return kz ? '📢 Объектті жылжыту үшін: сапалы фото, видео-тур, AI сипаттамасы.' : '📢 Для продвижения: качественные фото, видео-тур, AI-описание, публикация в Aira для коллег-риэлторов.';
  if(m.includes('риэлтор') || m.includes('выбрать') || m.includes('нанять')) return kz ? '🏆 Риэлторды рейтинг, сделок саны және мамандану бойынша таңдаңыз. ТОП риэлторлар бет бар!' : '🏆 Выберите риэлтора по рейтингу, количеству сделок и специализации. Перейдите в раздел «Риэлторы»!';
  if(m.includes('привет') || m.includes('сәлем') || m.includes('здравствуй')) return kz ? '👋 Сәлем! Мен Flai — AI-көмекшіңізмін. Пәтер табуға, ипотека есептеуге, баға беруге көмектесемін!' : '👋 Привет! Я Flai — ваш AI-помощник по недвижимости. Помогу найти жильё, рассчитать ипотеку, оценить объект!';
  if(m.includes('интересует') || m.includes('объект')) return kz ? '🏠 Жақсы! Бұл объект туралы толық ақпарат алу үшін риэлтормен байланысуды ұсынамын. Хабарласайын ба?' : '🏠 Отличный выбор! Рекомендую связаться с риэлтором для получения полной информации. Организовать звонок?';
...
function setComposeTab(tab) {
  ...
  if(!inp) return;
  var hints = { listing: 'Поделитесь объектом с коллегами: 3к, 85м², Есиль, 62 млн...', exchange: 'Предложите обмен: ищу 3к на 2к + доплата...', question: 'Задайте вопрос коллегам: как оформить ипотечную сделку?' };
...
function addAiraThread(txt, type) {
  ...
  var replyReplies = { listing: ['🤝 Есть покупатель! Пишу в личку', '👍 Интересный объект! Комиссию делим?', '📞 Клиент готов к ипотеке, свяжемся?'], exchange: ['🔄 Есть похожий вариант! Обсудим', '💡 Хороший обмен! Клиент экономит налог', '✅ Подходящий объект для обмена нашёл!'], question: ['💬 Делился опытом: нужен паспорт и СНТ', '📋 КПП — паспорт, справка с работы, СНТ', '🙋 Помогу! Писал по ипотеке в личку'] };
  ...
  div.innerHTML = '<div class="th-head" onclick="toggleThread(this)">' + '<div class="th-ava" style="background:'+rndC+'">'+ini+'</div>' + '<div style="flex:1">' + '<div class="th-name">'+esc(name.split(' ')[0])+typeTag+' <span class="th-time">только что</span></div>' + '<div class="th-prev">'+esc(txt.substring(0,50))+(txt.length>50?'...':'')+'</div>' + '</div>' + '<i class="fas fa-chevron-down" style="color:var(--t3);font-size:11px;transition:transform .2s"></i>' + '</div>' + '<div class="th-body">' + (type==='listing' ? '<div class="prop-tag"><i class="fas fa-home"></i> '+esc(txt.substring(0,60))+'</div>' : '') + '<p style="font-size:12px;color:var(--t2);margin-bottom:8px">'+esc(txt)+'</p>' + '<div id="aira-replies-'+Date.now()+'" style="font-size:12px;margin-bottom:8px"></div>' + '<div style="display:flex;gap:6px">' + '<button onclick="replyAira(this)" style="padding:5px 10px;border-radius:7px;background:var(--navy);color:#fff;font-size:11px;font-weight:600;cursor:pointer">💬 Ответить</button>' + '<button onclick="callRealtor(\''+esc(curUser?curUser.phone||'+7 701 000 00 00':'+7 701 000 00 00')+'\')" style="padding:5px 10px;border-radius:7px;background:var(--bg3);color:var(--t1);font-size:11px;font-weight:600;cursor:pointer;border:1px solid var(--brd2)">📞 Позвонить</button>' + '</div>' + '</div>';
...
function replyAira(btn) {
  ...
  form.innerHTML = '<textarea style="flex:1;padding:7px 10px;border-radius:8px;border:1.5px solid var(--brd);background:var(--bg);font-size:12px;resize:none;min-height:36px;font-family:inherit;color:var(--t1)" placeholder="Ваш ответ..."></textarea>' + '<button onclick="submitAiraReply(this)" style="width:36px;height:36px;border-radius:8px;background:var(--orange);color:#fff;font-size:14px;cursor:pointer;flex-shrink:0;align-self:flex-end;display:flex;align-items:center;justify-content:center"><i class="fas fa-paper-plane"></i></button>';
...
function replyThread(btn, realtorId, realtorName) { replyAira(btn); }
...
function genAI() {
  ...
  fetch('/api/ai/describe', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:type, rooms:rooms, area:area, district:dist, price:price, exchange:exch }) })
  .then(function(r){ return r.json(); })
  .then(function(d) {
...
function uploadMedia(type) {
  ...
  input.onchange = function() {
    ...
    if(type === 'photo') { toast('📷 '+input.files.length+' фото добавлено'); }
    else { toast('🎬 Видео добавлено ('+input.files[0].name+')'); var videoUrl = document.getElementById('a-video'); if(videoUrl) videoUrl.value = 'local:'+input.files[0].name; }
  };
  input.click();
}
function submitListing() {
  ...
  var newL = {
    id: Date.now(), type: type, rooms: rooms, area: parseInt(area), district: val('a-district') || 'Есиль', city: val('a-city') || 'Астана', price: parseInt(price),
    exchange: (document.getElementById('a-exch')||{}).checked || false,
    hasVideo: !!videoId, videoId: videoId,
    ...
    desc: val('a-desc') || 'Новый объект. Подробности по запросу.', photos: ['🛋️','🚿','🪟'],
  };
...
function renderCal() {
  ...
  var html = '<div class="cal-title">📅 '+t('nav_obj')+' / Расписание</div>' + '<div class="cal-date">'+dStr+'</div>' + '<div class="ai-tip"><span style="font-size:18px">🤖</span><span><b>Flai:</b> ' + (todayEv.length ? 'У вас '+todayEv.length+' событий сегодня. Не забудьте подготовиться!' : 'Сегодня событий нет. Хотите запланировать показ?') + '</span></div>' + '<button class="add-ev-btn" onclick="openM(\'m-ev\')"><i class="fas fa-plus"></i> Добавить событие</button>';
...
function renderProf() {
  ...
  el.innerHTML = '<div class="prof-hero">' + '<div class="ph-ava">'+ini+'</div>' + '<div class="ph-name">'+esc(curUser.name)+'</div>' + '<div class="ph-tag">🏠 Верифицированный риэлтор · '+(curUser.agency||'Астана')+'</div>' + '<div class="ph-stats">' + '<div class="ph-stat"><div class="ph-val">'+myListings.length+'</div><div class="ph-lbl">Объектов</div></div>' + '<div class="ph-stat"><div class="ph-val">⭐ '+(curUser.rating||4.8)+'</div><div class="ph-lbl">Рейтинг</div></div>' + '<div class="ph-stat"><div class="ph-val">'+(curUser.deals||0)+'</div><div class="ph-lbl">Сделок</div></div>' + '</div>' + '</div>' + '<div class="menu-sec"><div class="menu-lbl">Мои объекты</div>' + mItem('🏠','rgba(244,123,32,.1)','Активные объекты', myListings.length+' опубликованы',"toast('📋 '+"+myListings.length+"+' объектов активно')") + mItem('❤️','rgba(231,76,60,.1)','Избранное','Сохранённые объекты',"toast('❤️ Избранное — в разработке')") + '</div>' + '<div class="menu-sec"><div class="menu-lbl">Инструменты</div>' + mItem('📅','rgba(39,174,96,.1)','Планировщик','Показы и звонки',"go('s-cal');nav(null)") + mItem('🏆','rgba(244,123,32,.1)','Рейтинг риэлторов','Моя позиция',"closeM('m-more');go('s-realtors');nav(null);renderRealtors()") + mItem('🔄','rgba(39,174,96,.08)','Обмен недвижимостью','Актуальные запросы',"go('s-search');nav(document.getElementById('n-search'));setListTab('exch')") + mItem('💡','rgba(244,123,32,.08)','Налоговый советник 2026','Обмен vs продажа',"toast('💡 С 2026 года срок без налога — 2 года!')") + '</div>' + '<div class="menu-sec"><div class="menu-lbl">Aira — коллеги</div>' + mItem('💬','rgba(244,123,32,.1)','Чат риэлторов','47 коллег онлайн',"go('s-aira');nav(null)") + '</div>' + '<div class="menu-sec"><div class="menu-lbl">Аккаунт</div>' + mItem('⚙️','rgba(100,100,200,.08)','Настройки','Профиль, уведомления',"editProfile()") + '<div class="menu-item" onclick="doLogout()"><div class="menu-ico" style="background:rgba(231,76,60,.08)">🚪</div><div><div class="menu-name" style="color:#E74C3C">Выйти</div></div></div>' + '</div>';
}
function editProfile() { if(!curUser) return; toast('⚙️ Редактирование профиля — скоро будет!'); }
...
function applyLangUI() {
  ...
  var map = {
    'tx-tagline': t('tagline'), 'tx-flai-sub': t('flai_sub'), 'tx-flai-status': t('flai_status'),
    'tx-aira-sub': t('aira_sub'), 'tx-rel-header': t('rel_header'), 'tx-rel-sub': t('rel_sub'),
    'tx-notif-title': t('notif_title'), 'tx-menu-title': t('menu_title'), 'tx-today': t('today'),
    'tx-email-lbl': t('email_lbl'), 'tx-pass-lbl': t('pass_lbl'), 'tx-add-photo': t('add_photo'),
    'tx-add-video': t('add_video'), 'ld-sub': t('tagline'),
  };
