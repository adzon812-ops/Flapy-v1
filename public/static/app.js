...
/* ════════════════════════════════════════════════════
   🔐 SUPABASE CONFIG
═══════════════════════════════════════════════════ */
const SUPABASE_URL = 'https://xxxxx.supabase.co'; // ЗАМЕНИ НА СВОЙ
const SUPABASE_KEY = 'your-anon-key-here'; // ЗАМЕНИ НА СВОЙ
...
function renderFeed() {
  ...
  feed.innerHTML = listings.map((l) => {
    ...
    return `
      <div class="feed-card" style="height:100%;scroll-snap-align:start;scroll-snap-stop:always;position:relative;overflow:hidden;background:linear-gradient(135deg,#1a1a40,#0d1b3e)">
        ${hasMedia ? renderMediaLayer(l) : renderPlaceholder()}
        
        <div style="position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,0.1) 0%,transparent 25%,rgba(0,0,0,0.6) 70%,rgba(0,0,0,0.9) 100%);pointer-events:none"></div>
        ...
        <div style="position:absolute;bottom:0;left:0;right:0;padding:20px 16px 100px;color:white;z-index:10">
          ...
          <div style="display:flex;gap:10px;margin-top:12px">
            <button onclick="callRealtor('${l.phone}')" style="flex:1;padding:12px;background:#1E2D5A;color:white;border:none;border-radius:10px;font-weight:600;cursor:pointer">📞 ${WARM_MSG.call}</button>
            <button onclick="openWhatsApp('${l.whatsapp}', '${l.id}')" style="flex:1;padding:12px;background:#25D366;color:white;border:none;border-radius:10px;font-weight:600;cursor:pointer">💬 ${WARM_MSG.whatsapp}</button>
          </div>
        </div>
        
        ${l.photos.length > 1 ? `<div style="position:absolute;top:100px;right:16px;z-index:20;background:rgba(0,0,0,0.5);padding:4px 8px;border-radius:8px;font-size:11px;font-weight:600;color:white">${l.photos.length} фото</div>` : ''}
      </div>
    `;
  }).join('');
}
...
function renderMediaLayer(l) {
  ...
  if (l.tiktok) {
    return `<div style="width:100%;height:100%;position:absolute;inset:0;background:#000;display:flex;align-items:center;justify-content:center"><div style="text-align:center;color:white;padding:20px"><div style="font-size:48px;margin-bottom:12px">🎵</div><div style="font-weight:600">Видео-обзор</div></div><div style="position:absolute;bottom:20px;right:20px;opacity:0.3;font-weight:800;font-size:14px;color:white">FLAPY</div></div>`;
  }
  return '';
}

function renderPlaceholder() {
  return `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:120px;opacity:0.1">🏢</div>`;
}
...
function renderAuthSlot() {
  ...
  if (curUser) {
    ...
    slot.innerHTML = `<div style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:6px 12px;background:rgba(30,45,90,0.1);border-radius:20px" onclick="go('s-prof')"><div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#1E2D5A,#4A6FA5);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:14px">${name.charAt(0)}</div><span style="font-weight:600;color:#1E2D5A;font-size:13px">${name.split(' ')[0]}</span></div>`;
  } else {
...
function uploadMedia(type) {
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(evt) {
      ...
      document.getElementById('photo-upload-area').innerHTML = `<div style="font-size:32px;margin-bottom:8px">✅</div><div style="font-size:12px;color:var(--green);font-weight:600">Загружено: ${uploadedMedia.photos.length}/5</div>`;
...
function authTab(tab) {
  document.getElementById('tab-in').classList.toggle('on', tab === 'in');
  document.getElementById('tab-up').classList.toggle('on', tab === 'up');
