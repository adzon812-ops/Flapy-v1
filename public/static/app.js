  ...
  return [
    {
      ...
      tags: ['Новострой'],
      badge: 'Новое',
      desc: 'Просторная 3-комнатная с панорамным видом.',
      photos: ['🛋️', '🛁', '🪟', '🏗️']
    },
    {
      id: 2,
  ...
  return [
    {
      ...
      photo: 'А',
      color: '#1E2D5A',
      specialization: 'Квартиры, новострой',
      experience: 5,
      badge: 'ТОП',
      verified: true,
      reviewsList: [
        { author: 'Алия С.', stars: 5, text: 'Профессионал!', date: '15 янв' }
      ]
    },
    {
      id: 'r2',
...
function getFallbackCal() {
  ...
  return [
    {
      id: 1,
      title: 'Показ квартиры 3к Есиль',
      time: dt(0, 10, 0),
      type: 'showing',
      client: 'Алия С.',
      note: 'Взять ключи от 401',
      color: '#F47B20'
    }
  ];
}
...
function toggleLike(id, btn) {
  ...
  if (l.liked) btn.classList.add('liked');
...
function goChat(id) {
  ...
  closeM('m-det');
  go('s-flai');
  var nFlai = document.getElementById('n-flai');
  if (nFlai) nav(nFlai);
...
function updateNavVisibility() {
  // Placeholder for nav visibility logic
}
...
function sortRealtors(key, el) {
  document.querySelectorAll('.rsort').forEach(function(r) {
...
function renderRealtors() {
  ...
  var sorted = realtors.slice().sort(function(a, b) {
    return (b[realtorSort] || 0) - (a[realtorSort] || 0);
  });
  
  el.innerHTML = sorted.map(function(r, i) {
    return buildRealtorCard(r, i);
  }).join('');
}
...
function hireRealtor(realtorId, realtorName) {
  ...
  if (l) {
    ...
    if (r) {
      l.realtorId = r.id;
      l.realtor = r.name.split(' ')[0] + ' ' + (r.name.split(' ')[1] ? r.name.split(' ')[1].charAt(0) + '.' : '');
