...
// ✅ API: LISTINGS — ИСПРАВЛЕНО
app.get('/api/listings', async (c) => {
  try {
    const supabase = getSupabase(c)
    const { data, error } = await supabase.from('listings').select(`
      *,
      realtor:profiles!inner(full_name, agency, rating, deals, reviews, phone)
...
// ✅ API: REALTORS — ИСПРАВЛЕНО
app.get('/api/realtors', async (c) => {
  try {
    const supabase = getSupabase(c)
    const { data, error } = await supabase.from('profiles').select('*').eq('role', 'realtor').eq('verified', true).order('rating', { ascending: false }).limit(20)
